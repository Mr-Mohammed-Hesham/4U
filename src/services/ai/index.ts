...
export async function fetchGeneratedFlashcards(params: {
  ...
  if (viteApiKey) {
    try {
      const ai = new GoogleGenAI({ apiKey: viteApiKey });
      const prompt = `أنت خبير تعليمي ومؤلف مناهج. أنشئ قائمة بـ 6 بطاقات استذكار تكرار متباعد (Flashcards) لموضوع: "${topic}" في مادة "${subject || 'العلوم والرياضيات'}".
يجب أن تكون الأسئلة منطقية، علمية، دقيقة، وقائمة على المنهج، مع ذكر القوانين والصيغ الرياضية إن وجدت، وتفسير الحل خطوة بخطوة في الإجابة.
أرجع النتيجة بصيغة JSON حصرية بتنسيق:
[
  {
    "id": "c1",
    "front": "السؤال المنطقي الدقيق أو المسألة الحسابية",
    "back": "الحل النموذجي والتفسير العلمي المفصل",
    "category": "المفهوم / القانون",
    "formula": "الصيغة الرياضية إن وجدت"
  }
]`;
      const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
  ...
  // Dynamic Logical Subject-Specific Fallback Cards
  const cleanTopic = topic.trim();
  return [
    {
      id: 'gen_1',
      front: `ما هو المفهوم العلمي والقانون الأساسي الذي يحكم "${cleanTopic}"؟`,
      back: `يعتمد ${cleanTopic} على العلاقات الرياضية والفيزيائية المباشرة بين المتغيرات الأساسية، مع ضرورة تطبيق وحدات القياس الدولية (SI Units) والتأكد من شروط الاتزان والنظام.`,
      category: 'قانون ومفهوم رئيسي',
      formula: 'العلاقة الأساسية: F = f(x)'
    },
    {
      id: 'gen_2',
      front: `ما هي خطوات حل المسائل النموذجية المتعلقة بـ "${cleanTopic}" في الامتحانات؟`,
      back: `1. استخراج المعطيات والمطلوب وتحديد الرموز بدقة.\n2. تحويل الوحدات إلى النظام المتري الدولي.\n3. كتابة القانون الرياضي المناسب والتعويض فيه حسابياً.\n4. كتابة الناتج النهائي متبوعاً بوحدة القياس الصحيحة.`,
      category: 'استراتيجية الحل والامتحان'
    },
    {
      id: 'gen_3',
      front: `ما هي الشروط والافتراضات الفيزيائية / الرياضية الواجب توفرها لتطبيق قوانين "${cleanTopic}"؟`,
      back: `يجب التحقق من ثبوت العوامل المحددة (مثل درجة الحرارة، أو التسارع الثابت، أو اتزان القوى) وعدم وجود قوى إضافية مهملة لم تُدرج في المعادلة.`,
      category: 'شروط التطبيق'
    },
    {
      id: 'gen_4',
      front: `ما هي الأخطاء الشائعة التي يقع فيها الطلاب أثناء التعامل مع "${cleanTopic}"؟`,
      back: `إهمال إشارات الاتجاه (الموجب والسالب في الكميات المتجهة)، أو الخلط بين الوحدات الأساسية والفرعية، أو عدم تطبيق التربيع في المقامات ذات التربيع العكسي.`,
      category: 'تنبيه وأخطاء شائعة'
    }
  ];
}
